...
export const MainCard = ({getHidden,setHidden}) => {
  ...
  return (
    ...
    <div onClick={(e) => setHidden(false)} className=" space-y-16 sm:space-y-0 sm:grid grid-cols-[repeat(auto-fill,minmax(306px,1fr))]  overflow-hidden gap-x-6 gap-y-8 justify-items-center">
      {cardlist.map(({ id, img, logo, logoName, name  , prices, text }) => {
        return (
          <div key={id} className="md:max-w-[306px] w-full  group">
            <div className="relative w-full ">
              <div className="relative overflow-hidden rounded-lg ">
                <img className=' w-full transform group-hover:scale-110 transition duration-200 delay-100' src={img} alt="cardimg-01" />
              </div>
