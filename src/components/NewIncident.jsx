      ...
      <div className="flex items-center  justify-center sm:justify-between gap-3 relative py-3 flex-wrap ">
        <div className="flex  items-center gap-3">
          <div className="size-5 sm:size-10 rounded-full bg-white border border-border-bg flex items-center justify-center">
            <FontAwesomeIcon
              className="material-symbols-outlined text-[10px] sm:text-[20px] text-secondary "
              icon={faXmark}
            />
          </div>
          <div>
            <div>
              <div className="flex items-center justify-center gap-1   ">
                {array.map((item, i) => {
                  return (
                    <Fragment key={i}>
                      <h5 className="text-secondary text-[8px] sm:text-xs font-normal">
                        {item}
                      </h5>
                      {i === 2 ? (
                        ''
                      ) : (
                        <div className="w-1 sm:w-2  border border-secondary/60 "></div>
                      )}
                    </Fragment>
                  );
                })}
              ...
              <div>
                <h1 className="text-primary font-bold text-lg  sm:text-[26px]  mt-1">
                  New Incident
                </h1>
              </div>
            </div>
        ...
        <div className=" flex items-center justify-between gap-2 sm:gap-5">
          <Button
            onclick={Back}
            className="border-2 hover:bg-lime-400 transition delay-100 duration-150 ease-in-out  border-[D4D4D8] px-7 py-2 sm:px-14 sm:py-4  bg-white text-secondary font-bold text-xs"
          >
            Back
          </Button>
          {children}
